...
void AMannequin::BeginPlay()
{
	...
	//Si el Characte es poseido por Player, se asigna Gun a Mesh1P sino al Mesh de tercera persona
	if(IsPlayerControlled()){
		Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint")); //Attach gun mesh component to Skeleton, doing it here because the skelton is not yet created in the constructor
	}else {
		Gun->AttachToComponent(GetMesh(), FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
	}
	//Aignando las instancias de animacion para primera y tercera persona
...
void AMannequin::UnPossessed() {
	Super::UnPossessed();
	//cuando muere el Player se le asigna el Gun al cuerpo de tercera persona
	if(Gun != nullptr){
		Gun->AttachToComponent(GetMesh(), FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
	}
}
